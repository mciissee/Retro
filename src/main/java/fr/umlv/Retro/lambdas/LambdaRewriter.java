...
class LambdaRewriter implements Opcodes {
	...
	/**
	 * Converts invokedynamic to anonymous class creation.
	 * @param name the method's name.
	 * @param descriptor the method's descriptor (see Type).
	 * @param bsm the bootstrap method.
	 * @param bsmArgs the bootstrap method constant arguments.
	 */
	public void rewrite(String clazz, String name, String descriptor, Handle bsm, Object... bsmArgs) {
	 ...
	 */
	private void visitHeader(String name, String descriptor) {
	...
	private void visitFields(Type[] types) {
		IntStream.range(0, types.length).forEach(i -> {
			cw.visitField(ACC_FINAL | ACC_SYNTHETIC, "capture$" + i, types[i].toString(), null, null).visitEnd();
		});;
	}
	...
	private void visitConstructor(String clazz, String descriptor, Type[] params) {
		...
		IntStream.range(0, params.length).forEach(i -> {
			...
			mv.visitFieldInsn(PUTFIELD, clazz, "capture$" + i, params[i].toString());
		});
		mv.visitInsn(RETURN);
		mv.visitMaxs(1, 1);
		mv.visitEnd();
	...
	private void visitMethodImplements(String clazz, String name, Handle lambda, String interfacedescg, Type[] captures) {
		...
		for (int i = 0; i < captures.length; i++) {
			mv.visitVarInsn(ALOAD, 0);
			mv.visitFieldInsn(GETFIELD, clazz, "capture$" + i, captures[i].toString());
		}
	 ...
	 */
	private String createAccessMethod(Handle lambda) {
